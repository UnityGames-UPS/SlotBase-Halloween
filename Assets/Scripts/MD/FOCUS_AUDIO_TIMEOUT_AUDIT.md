...
### Known accepted limitation — occasional looping bg-track restart on refocus
`audioContext.suspend()` freezes the WebAudio clock for **every** source at once — it's the shared
context, so there's no way to suspend "just the SFX sources" and leave a looping background track's
clock running from the jslib side (Unity doesn't expose per-source WebAudio nodes in a stable,
cross-version way you could hook into from outside). This creates a real but narrow side effect: if
focus is lost/regained near a loop's start/end boundary, especially after a longer time backgrounded,
Unity's internal loop-scheduling can disagree with the resumed context clock about whether the current
loop iteration already finished, and restart the looping source from `0` instead of resuming mid-clip.

**This is intermittent by nature** (depends on exactly where in the loop you were and how long you
were backgrounded) and does not indicate a bug in `SetMuteAll`'s reentrancy guard or the JS listener
wiring — both are already idempotent regardless of timing. **Do not attempt to "fix" this by removing
or delaying `setUnityAudioSuspended`** — that reintroduces the ~3s audio-bleed bug this check exists
to prevent, which is the worse and more noticeable failure mode of the two. Treat an occasional
bg-loop restart-from-start on refocus as an accepted cost, not a regression to chase, unless a game's
bg track is long/noticeable enough that the restart itself becomes the bigger complaint — in which
case that's a per-game judgment call, not a default fix.
